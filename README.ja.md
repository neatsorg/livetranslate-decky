...
[Decky Loader](https://github.com/SteamDeckHomebrew/decky-loader) 用プラグイン。Gamescope/PipeWire経由で画面をキャプチャし、OCRで文字を認識して、リアルタイムで翻訳結果をオーバーレイ表示します。Steam Deckでのゲーム字幕翻訳を主な用途として作られていますが、多くの機能は、標準的な Linux 環境でも動作します。現在は、英語から日本語への翻訳に対応しています。
...
- 画面タップ翻訳 - リアルタイム自動翻訳を停止している際に、画面を指でタップした位置にある文章だけを翻訳表示します。
...
- キーバインド：画面の更新、一時停止と再開、タッチ翻訳用。それぞれの機能を、Steam Deck本体、ゲームパッド、キーボードなどに割り当てられます。各キーバインドは、長押しやキー同時押しに対応。
- レガシーOCR：TesseractOCRによるOCRエンジンも開発初期に作ったため、一応それで動かすこともできます。ただし今後メンテナンスの可能性は低いです。使用には当然ながらTesseractOCRが必要です。
...
ソースからのビルドについては[docs/BUILDING.md](docs/BUILDING.md) を参照してください(英語)。
...
- **ゲームパッドでクイックアクセスメニューをどうやって開くの**：HOMEボタン＋A。Xbox式ボタン配置の場合です。キーボードだとCtrl+2です。
